...
//    - If the inventory exists, update it and return a success message. If not, return a message indicating no data available.
    ...
    public Map<String, String> updateInventory(@RequestBody CombinedRequest combinedRequest){
        ...
        try{
            Long productId = combinedRequest.getProduct().getId();
            Long inventoryId = combinedRequest.getInventory().getId();
            //1. productId validation using service class
            boolean isValidProduct = serviceClass.ValidateProductId(productId);
            if(!isValidProduct){
                response.put("message", "Error: Product ID does not exist");
                throw new DataIntegrityViolationException("Error: Product Id not present");
            }
            //2. validate inventory. if exists update success
            boolean inventoryExists = serviceClass.validateInventory(combinedRequest.getInventory());
            if(!inventoryExists){
                ...
                throw new DataIntegrityViolationException("Error: Inventory not present");
            }
        }catch(Exception e){
...
//    - It first validates whether the inventory already exists. If it exists, it returns a message stating so. If it doesn’t exist, it saves the inventory and returns a success message.
    ...
    public Map<String, String> saveInventory(@RequestBody Inventory inventory){
        try{if(serviceClass.validateInventory(inventory)){
           inventoryRepository.save(inventory);
           return Map.of("message", "SUCCESS: inventory is saved");
        }else{
            return Map.of("message", "ERROR: The inventory already exists");
        }} catch (Exception e) {
...
//    - Return the filtered products in the response with the key `"product"`.
    ...
    public Map<String, Object> getProductName(@PathVariable("category") String category,@PathVariable("name") String name,@PathVariable("storeid") long storeid){
        List<Product> products = null;
        if(category == null || name == null){
            if(category == null){
                products = productRepository.findByNameLike(storeid, name);
            }else{
                products = productRepository.findProductByCategory(category, storeid);
            }
        }else{
...
//    - Returns a success message with the key `"message"` indicating successful deletion.
    ...
    public Map<String, String> removeProduct(@PathVariable("id") long id){
        if(serviceClass.ValidateProductId(id)){
            ...
            return Map.of("message", "Product successfully deleted from inventory");
        }else{
            return Map.of("message", "ERROR: Poduct does not present in the database");
        }
    }
